...
```{r}
#1. 
seqbyfour<-seq(1,100,4) # I used the seq(start#, to#, by#) function to generate a sequence from 1 and ending at 100, 
#specified with increments of four. 
...
#I created an object for the mean and specified the name of the value as seqbyfour_mean.
...
#I made an object for the median and specified name of the median value as seqbyfour_median. 
seqbyfour_median #calling the median in this line. The median is 49 for this sequence. 
#3. 
seqbyfour_mean>seqbyfour_median #I used the `>` conditional statement on the mean and median objects 
...
#so for grades lower than 50, function prints FALSE (meaning the student failed the test)
#for anything else (50 and above),function prints TRUE (meaning the student passed the test).
}
...
#Using TestResults_df[2,], which we learned in lab, I know Eda did not pass with score of 48. 
```
