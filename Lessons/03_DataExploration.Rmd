...
> Answer: When I load the tidyverse package I see other packages that are provided in tidyverse such as tibble, tidyr, and readr. 
...
# Don't have these folders on your own computer
...
#In the Console, you can also type ./ then TAB, hit TAB until you get where you want 
...
> ANSWER: You can find this file in the Data> Metadata folder. It is an MD file that is placed in a separate folder from the raw data. It provides an explanation of the raw data so future users are able to understand the data and conduct reproducible data analyses. 

```{r exploredata}
View(USGS.flow.data)
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data) # tells you if data is a data frame 
...
#c() is short for column bind 
str(USGS.flow.data) #str shows dataframe and all the objects you have on your dataframe
dim(USGS.flow.data) #short for dimension (how many rows and columns you have)
length(USGS.flow.data) #how many columns you have

head(USGS.flow.data) # head shows first six rows 
head(USGS.flow.data, 10) #first 10 observations
tail(USGS.flow.data, 5) #last 5 observations 
USGS.flow.data[30000:30005, c(3, 8, 14)] #get chunks of data

class(USGS.flow.data$datetime) #checks vector type: character
class(USGS.flow.data$discharge.mean) #numeric vector 
class(USGS.flow.data$gage.height.mean) #numeric vector
...
> Answer: Blank cells are imported as NA into R. 
...
> Answer: Removing NAs would be favorable for data recorded in the early stages of data that relies on data that is manually read and stored. Retaining NAs in the dataset is favorable if there are many NAs in the middle of your dataset and for time series analyses. 
...
```{r fixformartDate}
# 
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d") #transformed date into a simple character format

#
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d)
       }) #concatenating strings and characters together; d is the input (daytime column)
#
