...
st_crs(huc8_sf)
st_crs(huc8_sf)$epsg
# the CRS (coordinate reference system) is not the same as the counties dataset
#It uses the World Geodetic System  1984 CRS: 4326 (WGS84)
#The counties dataset uses the North American Datum 1983 (NAD83)
#
colnames(huc8_sf)
#columns include: FID (Feature ID), HUC_8 (Hydrologic Unit Code), SUBBASIN, ACRES, SQ_MILES, HU_8_STATE, DWQ_Basin, and geometry
#
st_bbox(huc8_sf)
...
huc8_filtered_sf <-st_read("./Data/Spatial/NCHUC8.shp") %>%
                    filter(SUBBASIN=="Upper Neuse")

otherhuc8_sf<-st_read("./Data/Spatial/NCHUC8.shp") %>%
                     filter(SUBBASIN!="Upper Neuse")
#Create a map
mapview(huc8_filtered_sf,col.regions='purple',
        alpha.regions=0.5,
        legend=FALSE) +
mapview(otherhuc8_sf, col.regions='yellow',
        alpha.regions=0.5,
        legend=FALSE)
...
Wind_energy<-read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv",
                      colClasses = c('HUC_12' = 'factor')) %>%
  mutate(HUC_8= substr(HUC_12,1,8)) %>%
  group_by(HUC_8) %>%
  dplyr::summarise(power= mean(kWhkm2day))
...
#View the outputs
mapview(huc8_sf_join, zcol="power")
...
mapview(the_huc_clip) + mapview(the_triangle,alpha.regions=0,
                                color='red')
