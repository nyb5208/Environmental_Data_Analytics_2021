...
class(NTL.nutrient.data.PeterPaul$sampledate) #currently factor-->change it to date
...
NTL.nutrient.data.PeterPaul.subset1<-select(NTL.nutrient.data.PeterPaul,lakename:sampledate,depth:po4)
...
We will work with the new functions from `tidyr`. For gather we will use `pivot_longer` and for spread we will use `pivot_wider`. Note that if you are familiar with `gather` and `spread` we may still use it, they are not going away, but they are not under active development.
...
dplyr::count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #returns # of different nutrient values 
...
NTL.nutrient.data.PeterPaul.spread2 <- pivot_wider(NTL.nutrient.data.PeterPaul.gathered, names_from = nutrient, values_from = concentration) #making rows into columns, make dataset wider than original 
...
LitterFiles
#list.files needs a path specified to folder with all files you want, read the csv files, use the full names of the files
Litter <- LitterFiles %>%
  ldply(read.csv)
```
...
NTL.PeterPaul.summaries <- 
  ...
  summarise(meantemp = mean(temperature_C), 
            ...
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#filter by depth 0 for surface water data, group by lake name and month, filter na's out of the following columns
