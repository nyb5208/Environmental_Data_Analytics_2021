...
NTL.phys.data.1990_thru_1999<-filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
...
NTL.phys.data.processed.Tuesday<-
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake" & year4 %in% c(1990:1999) & daynum %in% c(185:206))
...
NTL.Tuesday.summary<-
  ...
  summarise(meantemp_C =mean(temperature_C))
  # mean surface temp is 22.535 degrees Celsius
...
NTL.phys.data.irrad.gathered<-pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck,names_to="Sampling.Type",values_to="Irradiance.Value")
#remove NAs from irradiance column 
NTL.phys.data.irrad.gathered<-subset(NTL.phys.data.irrad.gathered, !is.na(Irradiance.Value) & !is.na(temperature_C))

#Exercise 6: spread temperatureC into more than one column based on the depth
NTL.phys.data.temp.subset<-select(NTL.phys.data.irrad.gathered,lakename:Irradiance.Value)
NTL.phys.data.temp.depth<-pivot_wider(NTL.phys.data.temp.subset,names_from=depth,values_from=temperature_C )
