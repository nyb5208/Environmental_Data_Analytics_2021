...
> Add notes here: We may be interested in long-term observations of temperature, oxygen, and light in lakes if we are monitoring them for eutrophication (N and P pollution) and want to assess the water health quality. 
...
library(lubridate)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) #stringsAsFactors=T allows for character summary to work

colnames(NTL.phys.data)
head(NTL.phys.data) #first 6 observations
summary(NTL.phys.data) #each column's data summarized
str(NTL.phys.data) #structure
dim(NTL.phys.data) #38614 rows and 11 columns
...
`<=` check values less than or equal to 
`>` check values greater than
`>=` check values greater than or equal to 
`&` check multiple conditions to see if both are true
...
#filtering by rows that have a zero on column depth

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) 
#filter by dataset, column depth equal to zero 

NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)
#filter by data and column depth less than 0.25
...
#yes, they all arrived to the same result. 
# Choose multiple conditions to filter
summary(NTL.phys.data$lakename)
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") # | is the or function
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") #!= excludes the lakes we are not interested in, not preferred when you have a lot you don't want 
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # %in% is based on more than two elements--type in the vector 
...
NTL.phys.data.1990_thru_1999<-filter(NTL.phys.data, year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: tidyverse does not preserve row numbers, which makes filtering by row_numbers inaccurate. 
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) # arranged in ascending order 
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth)) #arranged in descending order
...
#East Long Lake, 1998-07-16, depth 0.5, temp 34.1 C
#Hummingbird Lake, 2002-07-01, depth 0.0, temp 31.5 C
#Hummingbird Lake, 2002-07-19, depth 0.0, temp 29.0 C
#East Long Lake, 1995-06-19, depth 0.0, temp 28.5 C
#Hummingbird Lake 2002-07-01, depth 0.5, temp 28.5 C
#East Long Lake, 1995-06-19, depth 0.5, temp 28.3 C
```
### Select
...
#name of new column is temperature_F
...
#you can find the last row based on the index number from the dim()
#[1] 1984-05-27 UTC--2016-08-16 UTC
#if not ordered by date, you can use function first and function last
...
>We keep the wrangling code separate from the analysis code because this is just the initial processing step. We don't want everything in the same document because you can get lost.
