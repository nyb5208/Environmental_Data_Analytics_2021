...
> Answer: Null hypothesis: mean ozone AQI index >50
> Alternative hypothesis: mean ozone AQI index ≤ 50 
...
> P(t<-57.98)< 2.2e-16 and df=6829, we reject the null hypothesis and have sufficient evidence to suggest that ozone levels are below the threshold for good AQI index values.  
...
```{r}
#Exercise 2:
Ozone_year_density<-ggplot(EPAair,
                           aes(x=Ozone,
                               fill=as.factor(Year)))+
                           geom_density(alpha=0.5)+
                           labs(x="Mean Ozone AQI Index Value",
                                y="Density", fill="Year")
print(Ozone_year_density)
#
Ozone_year_boxplot<-ggplot(EPAair, 
                           aes(x=as.factor(Year), 
                               y=Ozone, 
                               color=as.factor(Year)))+
                    geom_boxplot()+
                    labs(x="Year",color="Year", y="Mean Ozone AQI Index Value")+
                    stat_compare_means()
print(Ozone_year_boxplot)
```
...
PM2.5_scatter<-ggplot(EPAair, aes(x=Ozone,y=PM2.5))+
               ...
               geom_text(x = 25, y = 300, label = PM2.5_Ozone_regression)
...
EPAairsub<-EPAair%>%
            ...
            na.omit()
EPAairsubcorr<-cor(EPAairsub)
corrplot(EPAairsubcorr)
...
AirAIC<-lm(data=EPAairsub, PM2.5~Ozone+Year+
             Month+mean_Latitude+mean_Longitude)
step(AirAIC)
...
newAir_model<-lm(formula = PM2.5 ~ Ozone + Month + mean_Latitude + mean_Longitude, 
    data = EPAairsub)
summary(newAir_model)
#R^2=0.19 > old R^2=0.13
```
