...
* "REST" is the framework of sending requests and handling responses - all in text.
 ...
 - REST is a form of an API (Application Programming Interface)
...
#Get the data, which starts on line 30
gage_data2 <- read.table(enoURL,skip = 29,header=TRUE,sep='\t') 

#Update the column headers
colnames(gage_data2) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")
#Tidy the data
gage_data_2 <- gage_data2 %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))
...
```{r}
#Apply the API key
#census_api_key("d130da7101a700f0691147676620fc5963d30722", install=TRUE)
nc_pop <- 
