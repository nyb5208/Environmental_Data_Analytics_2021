...
as.character(PeterPaul.chem.nutrients$year4)
#######
surface.temp.plot1<-ggplot(subset(PeterPaul.chem.nutrients,depth == 0), aes(x = daynum , y = temperature_C, color= year4))+geom_point()
print(surface.temp.plot1)
#
surface.temp.plot<-ggplot(subset(PeterPaul.chem.nutrients,depth == 0), aes(x = daynum , y = temperature_C, color= year4))+geom_point()+facet_wrap(vars(lakename), nrow = 2)+ylab("Surface Temperature (°C)")+xlab("Day of Year")+labs(color= "Year")
print(surface.temp.plot)
...
temp.date.plot<-ggplot(PeterPaul.chem.nutrients,  aes(x=sampledate, y=temperature_C,
  color=depth))+geom_point(alpha = 1, size = 0.5)+scale_color_viridis(option = "magma", direction = -1)+ylab("Temperature (°C)")+xlab("Sampling date")+
  scale_x_date(date_breaks = "5 years", date_labels = "%b %y") 
print(temp.date.plot)
...
AQI.ozone.plot<-ggplot(EPAair, aes(x= PM2.5,y= Ozone, color=mean_Latitude))+
  geom_point(alpha = 0.5, size =1.5)+ geom_smooth(method = lm)+
  ylab("Mean Ozone AQI value")+
  xlab("Mean PM2.5 AQI value")+
  labs(color="Average Latitude")

print(AQI.ozone.plot) 
...
EPAair$Month<-as.factor(EPAair$Month)
EPAair.month<-arrange(EPAair, Month)
AQIplot<-ggplot(EPAair, aes(x=Site.Name, y=PM2.5,color=Year,fill=Year))+
  geom_bar(stat="identity")+
  ylab("Mean PM2.5 AQI Value")
print(AQIplot)

AQIboxplot<-ggplot(EPAair, aes(x=COUNTY,y=PM2.5,fill=Year))+
  geom_boxplot()+ylab("Mean PM2.5 AQI Value")+xlab("County")
print(AQIboxplot)
